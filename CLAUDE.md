...
**Offerte PDF upload per config (2026-04-21, herzien 2026-04-22)** — Gedeelde UI in `assets/js/offertes-ui.js`: `renderOffertesCards(project)` + `renderOffertesSection(project)` (section-wrapped variant voor de drawer), `wireOffertesClicks(containerEl, getProjectFn, onChange)` voor delegated click-handling, en `openOfferteModal/closeOfferteModal/ensureOfferteModal` voor het upload-modal. Gebruikt door `dashboard.html` (in de drawer) én `project-edit.html` (als eigen blok onderaan). Upload-modal drag-drop, max 10 MB, PDF-only. Opslag: `project.offertes: { [configType]: { storagePath, filename, sizeBytes, contentType, uploadedAt, uploadedBy } }` top-level op het project-doc, blobs in Storage onder `projects/{id}/offertes/{configType}_{ts}.pdf` (dekt door bestaande `/projects/{id}/{allPaths=**}` Storage rule).

Per-config actions (congruent op beide pagina's):
- **Geen PDF**: `fa-upload` = open modal · `fa-trash` = dismiss (→ "Niet geoffreerd").
- **Met PDF**: `fa-download` · `fa-pen-to-square` vervang · `fa-file-circle-xmark` = **enkel PDF wissen, config blijft** · `fa-trash` = dismiss (PDF blijft bewaard zolang dismissed).
- **Dismissed (Niet-geoffreerd-lijst)**: `fa-rotate-left` = restore.

Dismiss-semantiek (2026-04-22 gewijzigd): `dismissProjectConfig` verwijdert de type **uit `lastCalcRun.inputs.selectedConfigTypes`** én voegt toe aan `dismissedConfigs` — zo verdwijnt de config ook uit de calculator. `restoreProjectConfig` doet het omgekeerde. `saveLastCalcRun` houdt de invariant "selectedConfigTypes disjoint van dismissedConfigs" automatisch aan: elk type dat opnieuw berekend wordt, wordt atomair uit `dismissedConfigs` gehaald (`arrayRemove(...selected)`). Gevolg: zodra de gebruiker op de calculator een eerder dismissed config opnieuw kiest en Bereken drukt, verliest hij zijn doorstrepping in drawer + project-edit.

Re-upload overschrijft de vorige blob atomair (nieuwe blob upload → Firestore swap → oude blob delete). Impliciete restore: `uploadProjectOfferte` haalt de type ook uit `dismissedConfigs`. `fa-triangle-exclamation` oranje warning-banner in drawer (en in project-edit) + parallel icoon in dashboard-rij/board-kaart wanneer `needsOfferteWarning(project)` true is. Helpers in `firebase-init.js`: `uploadProjectOfferte`, `deleteProjectOfferte` (PDF-only delete), `hardDeleteProjectConfig` (nog aanwezig maar niet meer via UI aangeroepen — gereserveerd voor admin via console), `dismissProjectConfig`, `restoreProjectConfig`, `needsOfferteWarning`. `mergeProjectMetadata` defaultet `offertes: {}` en `dismissedConfigs: []`. Calculator (`index.html`) blijft pure berekening — geen offerte-UI daar; share-links (`?s=`/`?data=`) tonen automatisch geen offertes want `_serializeState` kent de velden niet.
